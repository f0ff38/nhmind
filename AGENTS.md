...
7. **Гибридная координация** — Nostr (`RELAY_URL`, собственный relay на домене оператора) для публичного state/jobs; `_STD_.ws` — внутренний hot path (Phase 3+). Acurast P2P/tunnel relays и Substrate RPC **не** являются `RELAY_URL`. См. [docs/nostr-protocol.md](docs/nostr-protocol.md).
