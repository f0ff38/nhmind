...
3. **DNS** (зона в Selectel DNS) — после `apply`: workflow upsert **A** через [DNS API v2](https://docs.selectel.ru/en/api/dns-actual/). Токен: **IAM project name scope** ([get-selectel-dns-token.sh](../infra/selectel/scripts/get-selectel-dns-token.sh)) — secret **`SELECTEL_IAM_PROJECT_NAME`** (IAM → Проекты → **имя**, не hex id из Облачные серверы). `SELECTEL_PROJECT_ID` — только для Terraform/OpenStack. Опционально `RELAY_DNS_ZONE_ID` из URL панели.
...
├── get-selectel-dns-token.sh    # DNS API: IAM project name scope
...
| `SELECTEL_IAM_PROJECT_NAME` | **до DNS A** | IAM → Проекты → **имя** проекта (для DNS API v2; не путать с hex `SELECTEL_PROJECT_ID`) |
...
| DNS A: **zone not found** / **zero zones** | DNS v2 token scoped by cloud hex id | secret **`SELECTEL_IAM_PROJECT_NAME`** (IAM project **name**); опционально `RELAY_DNS_ZONE_ID` |
